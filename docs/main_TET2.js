...
document.querySelector("#play").addEventListener("click", async () => {
    ...
    for (let i = 0; i < mx; i++) {
        ...
        var dt_s = ( (sources[0].buffer.duration) * i ) + ( ( t_pad ) * i ) +  t_initBuffer;
        var dt_e = ( ( (sources[0].buffer.duration) * ( i + 1 ) ) + ( ( t_pad ) * i ) + 1 ) +  t_initBuffer;
...
document.querySelector("#stop").addEventListener("click", async () => {
    ...
    if (checkValue == answer){
        result = `<b>Correct 🎉</b> : ${answer} dB`;
    } else {
        result = `<b>Wrong 😭</b> : Correct answer is ${answer} dB`;
    }
